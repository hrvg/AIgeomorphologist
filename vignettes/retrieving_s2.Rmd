...
```{r pol}
one_random_polygon <- raster::buffer(one_random_point %>% 
	as("Spatial"), 500) %>% 
	raster::extent() %>% 
	as("SpatialPolygons") %>% 
	sf::st_as_sf() %>% 
...
s2_products <- s2_list(
	...
	time_interval = c(as.Date("2020-06-01"), as.Date("2020-10-01")),
	time_period = "full",
	level = "L2A",
	availability = "online",
	max_cloud = 0.1
...
# Set paths
out_dir  <- "F:/hguillon/research/data/s2/sen2r_output/" # output folder
safe_dir <- "F:/hguillon/research/data/s2/sen2r_safe/" # folder to store downloaded SAFE
